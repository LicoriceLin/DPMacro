...
from util import read_in,integrated_residue_iterator,sequence_from_object
...
def residue_within_threshold(entity1:allowed_residue_source,entity2:allowed_residue_source,threshold)->Set[Residue]:
    '''
    '''
    return set(integrated_residue_iterator(
...
# class structure_aligner:
    ...
    def _find_ligands(self):
        self.ligands=[i for i in integrated_residue_iterator(self.reference) 
                        if i.id[0] not in [' ','W']]
    def _find_proteins(self):
        self.proteins=[i for i in integrated_residue_iterator(self.reference) 
                        if i.id[0]==' ']
...
class RNAStructureAligner(ResidueFeatureExtractor):
# class structure_aligner:
    def __init__(self):
        ResidueFeatureExtractor.__init__(self,operation_name='AFill_aligner', 
                                            canonical_only=False)
    def _set_structure(self,struct:Union[str,Structure])->None:
        ...
        '''
        if isinstance(struct,str):
            self.full_object=read_in(struct,struct)
        elif isinstance(struct,Structure):
            self.full_object=struct.copy()
        else:
            raise TypeError
    def select_frame(self,modelid:int):
        self.object=self.full_object[modelid]
        self.sequences=sequence_from_object(self.object)
    def _set_reference_structure(self,struct:Union[str,Structure])->None:
        '''
        '''
